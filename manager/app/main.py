...
def addItem():
    if request.method == "POST":
        ...
        if image and allowed_file(image.filename):
            ...
            if not s3_config.validate_bucket_exists(s3, 'ece1779_a3_bucket'):
                s3_config.create_bucket(s3,'ece1779_a3_bucket')
            s3_config.store_data(s3,'ece1779_a3_bucket','products/'+str(productId)+"_"+filename, image)
        else:
            flash("Please upload an image")
            return redirect(url_for('add_product'))
